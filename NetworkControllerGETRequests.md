...
For some special cases, for example, when using [**aggregations** (see pt. 11)](#11-Get-List-Aggregation), it's more convenient to completely remove the data in the response.
...
In every request, filters can be defined to get a response, containing entries, which match the filtering condition. Use the `filter` parameter in the GET request URL.
...
This will translate to `date > 2020-10-06`. 

Allowed attribute filter operator values are:
...
For complex objects, filtering by relation(s) to other objects is also supported. The `filter` must be defined as double-array, containing the name of the relation as the first key, and operator in the second key.

```
/api/orders?filters[invoice][has]
```

This will only add `orders` to the list, which have a related `invoice`

- The filter can be extended further, by specifying a value to it:

```
/api/orders?filters[invoice][has]=1
```

This will only add `orders` to the list, which have a related `invoice`, where the `id` of the `invoice` is **1**.

Allowed relation filter operator values are:
...
```
/api/orders?resolve[]=lines.item.measure
```

The `lines` will be added to the response body, along with their related `item`.
...
If translations are supported in the system, the API will return the objects, with their translate-able attributes already translated to the selected language.

 - To get the translations of a Single Object, use the `/translations` endpoint 

```
/api/items/3/translations
```

This will return all translate-able properties of the object in all available locales:
```json
{
    "data": {
        "name": {
